...
import { calculateDays } from "../src/js/calculateDays.js"

describe("calculateDays (誕生日からの経過日数)", () => {
  it("誕生日と同じ年月日の場合は 1 日を返す（当日を 1 日目として数える）", () => {
    const birthDate = new Date(2000, 0, 1, 0, 0, 0)
    const today = new Date(2000, 0, 1, 23, 59, 59)

    const result = calculateDays(birthDate, today)
  ...
  it("today 引数を省略した場合は現在日時を基準に計算する", () => {
    ...
    const result = calculateDays(birthDate)
